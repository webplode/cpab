...
func RegisterAdminRoutes(r *gin.Engine, db *gorm.DB, jwtCfg config.JWTConfig, configPath string, cfg *sdkconfig.Config, baseHandler *sdkhandlers.BaseAPIHandler, authRateLimiter *ratelimit.Manager, coreManager *coreauth.Manager) {
	...
	authed.POST("/proxies/batch-delete", proxyHandler.BatchDelete)
	authed.GET("/proxies", proxyHandler.List)
	authed.POST("/proxies/:id/check", proxyHandler.Check)
	...
	modelMappingHandler := handlers.NewModelMappingHandler(db, r)
	...
	authed.POST("/model-tests", modelMappingHandler.TestModel)
	authed.POST("/model-tests/batch", modelMappingHandler.BatchTestModels)
