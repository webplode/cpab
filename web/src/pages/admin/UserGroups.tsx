...
function UserGroupModal({ title, initialData, submitting, onClose, onSubmit }: UserGroupModalProps) {
    const { t } = useTranslation();
    const [formData, setFormData] = useState<UserGroupFormData>(initialData);
...
function buildFormData(group?: UserGroup): UserGroupFormData {
    if (!group) {
        return {
            name: '',
            is_default: false,
...
export function AdminUserGroups() {
    ...
    const canListGroups = hasPermission(buildAdminPermissionKey('GET', '/v0/admin/user-groups'));
    const canCreateGroup = hasPermission(buildAdminPermissionKey('POST', '/v0/admin/user-groups'));
    const canUpdateGroup = hasPermission(buildAdminPermissionKey('PUT', '/v0/admin/user-groups/:id'));
    const canDeleteGroup = hasPermission(buildAdminPermissionKey('DELETE', '/v0/admin/user-groups/:id'));
    const canSetDefault = hasPermission(
        buildAdminPermissionKey('POST', '/v0/admin/user-groups/:id/default')
    );
    ...
    const fetchGroups = useCallback(() => {
        ...
        setLoading(true);
        apiFetchAdmin<ListResponse>('/v0/admin/user-groups')
    ...
    const handleCreate = async (payload: Record<string, unknown>) => {
        ...
        setSubmitting(true);
        try {
            await apiFetchAdmin('/v0/admin/user-groups', {
                method: 'POST',
    ...
    const handleUpdate = async (payload: Record<string, unknown>) => {
        ...
        setSubmitting(true);
        try {
            await apiFetchAdmin(`/v0/admin/user-groups/${editGroup.id}`, {
                method: 'PUT',
                body: JSON.stringify(payload),
            });
            setGroups((prev) =>
    ...
    const handleSetDefault = async (group: UserGroup) => {
        ...
        try {
            await apiFetchAdmin(`/v0/admin/user-groups/${group.id}/default`, {
                method: 'POST',
            });
            fetchGroups();
        } catch (err) {
    ...
    const handleDelete = async (group: UserGroup) => {
        ...
        setConfirmDialog({
            ...
            message: t(
                'Are you sure you want to delete user group #{{id}}? This action cannot be undone.',
                { id: group.id }
            ),
            ...
            onConfirm: async () => {
                try {
                    await apiFetchAdmin(`/v0/admin/user-groups/${group.id}`, { method: 'DELETE' });
                    fetchGroups();
                } catch (err) {
                    console.error(err);
                } finally {
    ...
    const pageInfo = useMemo(() => {
        if (!groups.length) return t('No user groups found');
        const start = (currentPage - 1) * PAGE_SIZE + 1;
        const end = Math.min(currentPage * PAGE_SIZE, groups.length);
        return t('Showing {{from}} to {{to}} of {{total}} user groups', {
            from: start,
            to: end,
            total: groups.length,
        });
    ...
    if (!canListGroups) {
        return (
            <AdminDashboardLayout
                title={t('User Groups')}
                subtitle={t('Configure default groups and manage assignments.')}
            >
                <AdminNoAccessCard />
            </AdminDashboardLayout>
        );
    ...
    return (
        ...
        >
            ...
            {editGroup && (
                <UserGroupModal
                    title={t('Edit User Group #{{id}}', { id: editGroup.id })}
